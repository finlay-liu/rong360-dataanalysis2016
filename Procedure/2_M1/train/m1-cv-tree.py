...
def P_YYYY(N_train, target_train, N_test, target_test):
    ...
    #print clf.score(N_train, target_train)
...
#得到决策树的信息，写在了treeInfo.dot这个文件下，用Graphviz这个软件再绘出图
with open("Procedure_dta\\treeInfo.dot", 'w') as f:
    f = tree.export_graphviz(clf,feature_names=train.ix[0:,2:].columns,out_file=f)
